...
class OptimizedCubeDetector:
    ...
    def classify_symbol(self, symbol_img: np.ndarray) -> Tuple[str, float]:
        ...
        # 2. KHÔNG CHIA 255, ÉP KIỂU UINT8
        img = img.astype(np.uint8) 
        
        # 3. Thêm chiều batch và channel: (64,64) -> (1, 64, 64, 1)
        img = np.expand_dims(img, axis=0) # Thêm batch -> (1, 64, 64)
        img = np.expand_dims(img, axis=-1) # Thêm channel -> (1, 64, 64, 1)
